...
"""Classes for handling connected/ordered schemes formed by multiple state together with information on kinetic schemes"""
import ihm.multi_state_scheme


class MultiStateScheme(object):
    """MultiStateScheme collects information about a collection of multiple states ,
       that can form a connected/ordered scheme.
       A special case is a kinetic scheme, for which kinetic rates and relaxation times are available.

       :param str name: The name of the multi-state scheme.
       :param str details: Details on the scheme.
       :param list_of_connectivities: A list of connectivities that belong to the scheme.
       :type list_of_connectivities: List of :class:`ìhm.MultiStateSchemeConnectivity`
       :param list_of_relaxation_times: A list of relaxation times not assigned to specific connectivities, but to the scheme
    ...
    def __init__(self, name, details=None, list_of_connectivities=None,
        ...
        self.states = [] # This is filled automatically based on the connectivity_list
    ...
    def add_connectivity(self, connectivity):
        ...
        if connectivity not in self.connectivity_list:
            # Make sure that the connectivity has not been assigned to another scheme
            if not connectivity._assigned_to_scheme:
                connectivity.set_assigned_to_scheme()
                self.connectivity_list.append(connectivity)
            # If the connectivity has beedn assigned to another scheme, create a copy of the connectivity and use that
            else:
                old_connectivity = connectivity
                connectivity = ihm.multi_state_scheme.MultiStateSchemeConnectivity(
        ...
        if (connectivity.end_state is not None) and (connectivity.end_state not in self.states):
...
class MultiStateSchemeConnectivity(object):
    """A connectivity between states. Used to describe the directed edge of graph.
    If no end_state is given, the state is not connected to another state. This could be the case for states
    where no connection to other states could be resolved.

    :param begin_state: The start state of the connectivity.
    :type begin_state: :class:`ihm.model.State`
    :param end_state: The end state of the connectivity. Can be None in case of states that are not connected to others.
    :type end_state: :class:`ìhm.model.State`
    :param details: Details to the connectivity.
    :param dataset_group: The DatasetGroup that was used to obtain information on the connectivity.
    ...
    def __init__(self, begin_state, end_state=None, details=None,
        ...
        # The _assigned_to_scheme variable tracks whether the connectivity has been assigned to a scheme.
        # This is to ensure that each connectivity is only assigned to a single scheme.
...
class KineticRate(object):
    ...
    The kinetic rate could be either a transition_rate_constant or an equilibrium_constant.
    ...
    :param float transition_rate_constant: A transition rate constant describing the exchange between two states. Unit: per second.
    :param float equilibrium_constant: An equilibrium constant describing the exchange between two states
    :param str equilibrium_constant_determination_method: The method how the equilibrium_constant was determined.
    ...
    :param str equilibrium_constant_unit: The unit of the equilibrium constant. Depending on what the process described,
      a unit might be applicable or not.
    ...
    :param external_file: An external file containing measurement data for the kinetic rate.
    ...
    def __init__(self, transition_rate_constant=None,
                 ...
                 dataset_group=None,
                 external_file=None):
        self.allowed_equilibrium_constant_determination_methods = [
        ...
        # check whether the equilibrium_constant_determination_method is within the list of options if an equilibrium constant is given
        if self.equilibrium_constant is not None:
            if equilibrium_constant_determination_method not in self.allowed_equilibrium_constant_determination_methods:
                raise ValueError("Error in KineticRate: Equilibrium determination method \"%s\" "
                                 "is not in the list of allowed values. Options are: %s"%(
                    equilibrium_constant_determination_method,
                ";".join(["\'%s\'"%(x) for x in self.allowed_equilibrium_constant_determination_methods])))
        self.equilibrium_constant_determination_method = equilibrium_constant_determination_method
        self.details = details
        self.dataset_group = dataset_group
...
class RelaxationTime(object):
    """A relaxation time determined for a scheme. The relaxation time can either be connected to a specific connectivity
    in the scheme or to the scheme in general if no assignment is possible.

    :param float value: The relaxation time.
    :param str unit: The unit of the relaxation time. Options are ['seconds','milliseconds', microseconds']
    :param float amplitude: The amplitude of the relaxation time if determined.
    :param str details: Details on the relaxation time.
    :param dataset_group: The DatasetGroup used to determine the relaxation time.
    :type dataset_group: :class:`ihm.dataset.DatasetGroup`
    :param external_file: An external file containing measurement data for the relaxation time.

    """
    def __init__(self, value, unit, amplitude=None,
                 details=None, dataset_group=None, external_file=None):
        self.list_of_allowed_relaxation_time_units = ['seconds','milliseconds','microseconds']
