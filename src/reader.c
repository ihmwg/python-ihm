...
/* Temporary hack - should migrate to GError* */
#define mod_logerror printf

/* todo - don't ignore error */
static gboolean file_read_line(GIOChannel *fh, GString *line, gboolean *eof)
{
  gsize terminator;	 
  GIOStatus stat = g_io_channel_read_line_string(fh, line, &terminator, NULL);
  if (stat == G_IO_STATUS_ERROR || stat == G_IO_STATUS_AGAIN) {
    /* report error (todo: handle AGAIN sensibly) */
    return FALSE;
  } else {
...
static void set_value(struct mmcif_reader *reader,
                      ...
                      gboolean own_data, int *ierr)
{
  if (key->in_file) {
    mod_logerror("mmcif_read_line", 
...
static size_t get_next_token(struct mmcif_reader *reader, char *line,
                             size_t len, size_t start_pos, int *ierr)
{
  ...
  if (*pt == '\0') {
    ...
    return handle_quoted_token(reader, line, len, start_pos, "Double", ierr);
  } else if (*pt == '\'') {
    return handle_quoted_token(reader, line, len, start_pos, "Single", ierr);
  } else if (*pt == '#') {
...
static void mmcif_tokenize(struct mmcif_reader *reader, char *line, int *ierr)
{
  ...
  *ierr = 0;
  ...
  for (start_pos = 0; start_pos < len && *ierr == 0;
       start_pos = get_next_token(reader, line, len, start_pos, ierr)) {
  }
  if (*ierr) {
    g_array_set_size(reader->tokens, 0);
  }
...
static void read_multiline_token(struct mmcif_reader *reader,
                                 gboolean ignore_multiline, int *ierr)
{
  int eof = 0;
  ...
  *ierr = 0;
  while (!eof) {
    reader->linenum++;
    if (!file_read_line(reader->fh, reader->nextline, &eof)) {
      *ierr = 1;
      return;
  ...
  *ierr = 1;
  mod_logerror("mmcif_read", 
               "End of file while reading multiline string\n"
               "              which started on line %d",
               start_linenum);
}
  ...
  *ierr = 0;
  if (reader->tokens->len <= reader->token_index) {
    do {
      ...
      reader->linenum++;
      if (!file_read_line(reader->fh, reader->line, &eof)) {
        *ierr = 1;
        return NULL;
      } else if (reader->line->len > 0 && reader->line->str[0] == ';') {
        read_multiline_token(reader, ignore_multiline, ierr);
        if (*ierr) {
          return NULL;
        }
      } else {
        mmcif_tokenize(reader, reader->line->str, ierr);
        if (*ierr) {
          return NULL;
        } else {
          reader->token_index = 0;
                                   ...
                                   char **keyword, int *ierr)
{
  ...
  if (!dot) {
    mod_logerror("mmcif_read", 
                 "No period found in mmCIF variable name (%s) in file, line %d",
                 str, reader->linenum);
    *ierr = 1;
    return;
  }
...
static void mmcif_read_value(struct mmcif_reader *reader,
                             struct mmcif_token *key_token, int *ierr)
{
  struct mmcif_category *category;
  ...
  if (category) {
    ...
    if (key) {
      struct mmcif_token *val_token = mmcif_get_token(reader, FALSE, ierr);
      if (val_token && val_token->type == MMCIF_TOKEN_VALUE) {
        set_value(reader, category, key, val_token->str, TRUE, ierr);
      } else if (*ierr == 0) {
        mod_logerror("mmcif_read_value", 
                                               ...
                                               gboolean first_loop, int *ierr)
{
  ...
  if (first_loop) {
    *catpt = category;
    ...
    mod_logerror("mmcif_read_loop", 
                 "mmCIF files cannot contain multiple categories "
                 "within\n              a single loop in file, line %d",
                 reader->linenum);
    *ierr = 1;
    return NULL;
  }
                          ...
                          int *ierr)
{
  if (category->callback) {
    ...
    if (force) {
      (*category->callback) (reader, category->data, ierr);
    }
  }
  ...
  *ierr = 0;
  while (*ierr == 0 && (token = mmcif_get_token(reader, FALSE, ierr))) {
    if (token->type == MMCIF_TOKEN_VARIABLE) {
      ...
      mod_logerror("mmcif_read_loop", 
                   "Was expecting a keyword or value for loop, in file, line %d",
                   reader->linenum);
      *ierr = 1;
    }
  }
...
static void mmcif_read_loop_data(struct mmcif_reader *reader,
                                 ...
                                 struct mmcif_keyword **keywords, int *ierr)
{
  *ierr = 0;
  while (*ierr == 0) {
    ...
    int i;
    for (i = 0; *ierr == 0 && i < len; ++i) {
      struct mmcif_token *token = mmcif_get_token(reader, FALSE, ierr);
      if (*ierr) {
        break;
      } else if (token && token->type == MMCIF_TOKEN_VALUE) {
        if (keywords[i]) {
          set_value(reader, category, keywords[i], token->str, !oneline, ierr);
        }
      } else if (i == 0) {
        /* OK, end of the loop */
        if (token) {
        ...
        mod_logerror("mmcif_read_loop", 
                     ...
                     "exact\n              multiple of the number of keys) "
                     "in file, line %d", reader->linenum);
        *ierr = 1;
      }
    }
    if (!*ierr) {
      call_category(reader, category, TRUE, ierr);
    }
  }
}

/* Read a loop_ construct from the file. */
static void mmcif_read_loop(struct mmcif_reader *reader, int *ierr)
{
  GPtrArray *keywords;
  struct mmcif_category *category;

  keywords = mmcif_read_loop_keywords(reader, &category, ierr);
  if (*ierr) {
    return;
  }
  if (category) {
    mmcif_read_loop_data(reader, category, keywords->len,
                         (struct mmcif_keyword **)keywords->pdata, ierr);
  }
  g_ptr_array_free(keywords, TRUE);
}
                                  ...
                                  gpointer user_data)
{
  ...
  if (*d->ierr == 0) {
    call_category(d->reader, value, FALSE, d->ierr);
  }
}

/* Process any data stored in all categories */
static void call_categories(struct mmcif_reader *reader, int *ierr)
{
...
void mmcif_read_file(struct mmcif_reader *reader, int *ierr)
{
  ...
  struct mmcif_token *token;
  *ierr = 0;
  while (*ierr == 0 && (token = mmcif_get_token(reader, TRUE, ierr))) {
    if (token->type == MMCIF_TOKEN_VARIABLE) {
      mmcif_read_value(reader, token, ierr);
      ...
      mmcif_read_loop(reader, ierr);
    }
  }
  call_categories(reader, ierr);
}
